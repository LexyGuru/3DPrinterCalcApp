...
export const Printers: React.FC<Props> = ({ printers, setPrinters, settings, theme, themeStyles }) => {
  ...
  const handleExportCSV = async () => {
    try {
      console.log("📤 CSV export indítása...", { printersCount: printers.length });
      
      const csvRows: string[] = [];
      csvRows.push("Név,Típus,Teljesítmény (W),Kopás (€/h),AMS szám");
      printers.forEach(p => {
        csvRows.push(`${p.name},${p.type},${p.power},${p.usageCost},${p.amsCount || 0}`);
      });
      const csvContent = csvRows.join("\n");

      const filePath = await save({
        defaultPath: `printers_${new Date().toISOString().split("T")[0]}.csv`,
        filters: [{
          name: "CSV",
          extensions: ["csv"]
        }]
      ...
      if (filePath) {
        ...
        showToast(
          settings.language === "hu" ? "Nyomtatók sikeresen exportálva CSV formátumban" :
          settings.language === "de" ? "Drucker erfolgreich als CSV exportiert" :
          "Printers exported successfully as CSV",
          "success"
        );
      } else {
  ...
  const handleImportCSV = async () => {
    try {
      ...
      const importedPrinters: Printer[] = [];
      for (let i = 1; i < lines.length; i++) {
        const columns = lines[i].split(",").map(col => col.trim());
        if (columns.length >= 5) {
          const name = columns[0];
          const type = columns[1];
          const power = Number(columns[2]);
          const usageCost = Number(columns[3]);
          const amsCount = Number(columns[4]) || 0;

          if (name && type && !isNaN(power) && power > 0 && !isNaN(usageCost) && usageCost >= 0) {
            importedPrinters.push({
              id: Date.now() + i,
              name,
              type,
              power,
              usageCost,
              amsCount: amsCount > 0 ? amsCount : undefined
            });
          }
        }
      }

      if (importedPrinters.length === 0) {
        throw new Error("Nem sikerült importálni egyetlen nyomtatót sem");
      }

      console.log("✅ CSV import sikeres", { importedCount: importedPrinters.length });
      setPrinters([...printers, ...importedPrinters]);
      showToast(
        settings.language === "hu" ? `${importedPrinters.length} nyomtató sikeresen importálva` :
        settings.language === "de" ? `${importedPrinters.length} Drucker erfolgreich importiert` :
        `${importedPrinters.length} printers imported successfully`,
        "success"
      );
    } catch (error) {
  ...
  const handleExportJSON = async () => {
    try {
      console.log("📤 JSON export indítása...", { printersCount: printers.length });
      
      const exportData = { printers };
      const jsonContent = JSON.stringify(exportData, null, 2);

      const filePath = await save({
        defaultPath: `printers_${new Date().toISOString().split("T")[0]}.json`,
        filters: [{
          name: "JSON",
          extensions: ["json"]
        }]
      ...
      if (filePath) {
        ...
        showToast(
          settings.language === "hu" ? "Nyomtatók sikeresen exportálva JSON formátumban" :
          settings.language === "de" ? "Drucker erfolgreich als JSON exportiert" :
          "Printers exported successfully as JSON",
          "success"
        );
      } else {
  ...
  const handleImportJSON = async () => {
    try {
      ...
      // Hozzáadunk ID-kat az importált nyomtatókhoz, ha nincs
      const importedPrinters = importData.printers.map((p: Printer, idx: number) => ({
        ...p,
        id: p.id || Date.now() + idx
      }));

      console.log("✅ JSON import sikeres", { importedCount: importedPrinters.length });
      setPrinters([...printers, ...importedPrinters]);
      showToast(
        settings.language === "hu" ? `${importedPrinters.length} nyomtató sikeresen importálva` :
        settings.language === "de" ? `${importedPrinters.length} Drucker erfolgreich importiert` :
