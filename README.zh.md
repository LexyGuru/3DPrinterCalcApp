...
### v1.9.0 (2025) - 🔍 系统诊断和性能改进

- 🔍 **系统诊断** - 全面的系统健康检查工具:
  - 系统信息显示 (CPU, 内存, OS, GPU, 磁盘)
  - 文件系统验证 (data.json, filamentLibrary.json, update_filament.json)
  - 模块可用性检查 (Settings, Offers, Printers, Customers, Calculator, Home)
  - 数据存储可访问性验证
  - 带有详细状态消息的进度条
  - 带有错误/警告/成功指标的摘要
  - 重新运行诊断按钮
...
- ⚡ **日志查看器性能** - 大型日志文件的虚拟滚动:
  - 为 LogViewer 组件定制虚拟滚动实现
  - 仅渲染可见的日志条目, 大幅提高性能
  - 即使对于大型日志文件 (100k+ 行) 也能流畅滚动和搜索
  - 保持准确的滚动条位置和高度
  - 显著加快搜索和筛选操作

- 🔔 **统一通知系统** - 集中式通知服务:
  - 单个 `notificationService` 用于 Toast 和平台通知
  - 基于优先级的通知路由 (高优先级 → 平台通知)
  - 基于应用状态 (前台/后台) 的自动决策
  - 与现有通知函数向后兼容
  - 可配置的通知偏好 (Toast 开/关, 平台通知开/关, 优先级级别)

- 🎯 **UI/UX 改进**:
  - 系统诊断从备份部分移至日志管理部分 (更合理的位置)
  - 修复 TypeScript linter 错误 (未使用的变量, 类型不匹配)
  - 提高代码质量和可维护性

### v1.8.0 (2025) - 📊 高级日志记录和恢复出厂设置增强

- 🔄 **恢复出厂设置进度模态** - 恢复出厂设置的视觉进度指示器:
...
- 📋 **完整的日志系统重构** - 专业日志基础设施:
  - 跨平台日志文件路径 (平台特定的数据目录)
  - 系统信息记录 (CPU, 内存, OS, GPU, 磁盘, 应用版本)
  - 目录信息记录 (日志和备份文件夹, 文件计数, 大小)
  - 详细的加载状态记录 (成功/警告/错误/严重)
  - 日志级别 (DEBUG, INFO, WARN, ERROR) 带筛选
  - 结构化日志格式支持 (文本和 JSON)
  - 带自动清理的日志轮换 (可配置的保留天数)
  - 带筛选、搜索、高亮和导出的日志查看器模态
  - 设置中的日志配置 (格式, 级别, 保留天数)
  - 日志文件内容在应用重启之间保留 (追加模式)
...
- 🛠️ **技术改进**:
  - 恢复出厂设置期间禁用日志记录以防止日志污染
  - data.json 创建延迟到语言选择 (更干净的恢复出厂设置流程)
  - 日志文件初始化延迟到语言选择
  - 语言选择后自动重启应用
  - 用于备份和日志文件管理的后端命令
  - 备份和日志的跨平台路径处理
  - 修复内存计算 (sysinfo 0.31 兼容性)
  - 修复 React 样式警告 (CSS 简写冲突)

### v1.7.0 (2025) - 💾 备份系统、加载屏幕和耗材库改进

- 💾 **完整的备份系统实现**
  - 每天一个备份文件的自动备份系统（仅在新的一天创建）
  - 备份提醒钩子和 UI 组件 - 当不存在备份时通知
  - 设置中的备份历史 UI - 颜色编码列表（绿色/黄色/红色/灰色）显示备份文件年龄和删除倒计时
  - 自动保存模态窗口 - 启用自动保存时的说明
  - 自动保存和自动备份同步 - 自动保存保存时自动备份
  - 恢复出厂设置现在删除自动备份文件
  - 启用自动保存时自动刷新备份历史
- 🔧 **备份系统后端优化**
  - 添加了用于删除旧备份的后端命令（`cleanup_old_backups_by_days`、`cleanup_old_backups_by_count`）
  - 更新了前端清理函数以使用后端命令，消除了"禁止路径"错误
  - 所有文件操作（创建、删除、列出）现在从后端进行，避免了 Tauri 权限问题
- ⚡ **备份系统性能优化**
  - `hasTodayBackup()` 已优化：使用后端命令 `list_backup_files`，无需读取所有文件
  - 添加了锁定机制以防止并行备份创建
  - 即使有大量备份文件也能更快运行
- 📁 **打开备份目录和日志历史**
  - 在设置 → 备份历史中添加了按钮以打开备份文件夹
  - 设置中的新日志历史部分 - 列出并打开日志文件
  - 可按天数配置自动删除日志文件
  - 跨平台支持（macOS、Windows、Linux）
- 🎨 **完全重新设计加载屏幕**
  - 应用徽标集成为背景，带有玻璃态效果
  - 固定布局用于复选标记 - 自动滚动，一次仅显示 3 个模块
  - 闪烁效果，脉动点动画
  - 带有隐藏滚动条的滚动容器
- ⚙️ **加载过程改进**
  - 减慢加载速度（800ms 延迟）- 加载消息可读
  - 每个模块的错误处理（try-catch 块）
  - 所有状态和错误的物理日志文件
  - 最后加载摘要
- 🎨 **耗材库多语言支持**
  - 在所有支持的语言中显示耗材颜色（不仅仅是匈牙利语/德语/英语）
  - 回退逻辑：英语 → 匈牙利语 → 德语 → 原始颜色/名称
  - 设置、全局搜索和耗材组件已更新
- 🔄 **恢复出厂设置改进**
  - 删除物理文件（`data.json`、`filamentLibrary.json`、`update_filamentLibrary.json`）
  - 无需重新加载的存储实例重置
  - 恢复出厂设置后显示语言选择器
- 🎓 **教程使用 v1.7.0 功能更新**
  - 新步骤：widget-interactivity、table-sorting、autosave-backup、filament-library-multilang
  - 扩展的演示数据：6 个耗材 → 11 个耗材，3 个报价 → 5 个报价
  - 为所有语言添加了翻译键
